...
is not in the table, open or not: it appears when you **hold Ctrl — ⌥ Option on a Mac — with the
pointer on the entity's header**. In the compact table, a column's description appears the same way
**on its row**; an opened entity already writes it out, so its rows show nothing more. Without the
key, hovering only lights the entity and its relationships, as before: a description is there when
asked for, not over the diagram at every pass of the mouse. Pressing the key with the pointer
already in place works too, and letting it go hides the description. A diagram with descriptions
says so in the note under it.
